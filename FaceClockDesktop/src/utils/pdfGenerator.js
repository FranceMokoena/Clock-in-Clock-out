/**
 * 📄 PDF Generator Utility for Desktop (Electron + React)
 * 
 * Generates PDFs for dashboard exports
 * Uses browser-compatible html2pdf library
 */
...
export const generateDashboardPDF = async (stats, userName = 'Admin') => {
  try {
    ...
    // Use window.print() or save to file in Electron environment
    const fileName = `Dashboard_${new Date().toISOString().split('T')[0]}_${Date.now()}.pdf`;
    
    // For Electron, we can use window.print() or trigger a download
    // The simplest approach is to create a new window and print from there
    const printWindow = window.open('', '', 'width=800,height=600');
    printWindow.document.write(htmlContent);
    printWindow.document.close();
    printWindow.print();
    
    return { success: true, filePath: fileName, message: `PDF export initiated!` };
  } catch (error) {
